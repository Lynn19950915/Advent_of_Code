...
<template>
    <div id="sidebar">
        ...
        <div id="pagination">
            <span v-show="currentPage>1" class="pageClick" @click="currentPage--">{{ "<" }}</span>
            <span style="padding: 0 15px">
                {{ (currentPage-1)*10+1 }} - {{ Math.min(currentPage*10, days.length) }}
            </span>
            <span v-show="currentPage<3" class="pageClick" @click="currentPage++">{{ ">" }}</span>
        </div>
    </div>
</template>
...
#sidebar {
    ...
    font-size: 15px;
    /* 頂端列消失後，就固定住側欄 */
    top: 10px; position: sticky; align-self: flex-start;
}
a {
